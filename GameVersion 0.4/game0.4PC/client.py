            ...
            if p_id != p.id:
                ...
                for bullet in list(enemy.bullets):
                    ...
                    if my_rect.colliderect(b_rect):
                        ...
                        bullets_to_remove = []
                        # Итерируемся по копии списка пуль нашего игрока, чтобы безопасно их удалять
                        for bullet in list(p.bullets): 
                            # Создаем прямоугольник коллизии для пули
                            b_rect = pygame.Rect(bullet[0]-6, bullet[1]-6, 12, 12)
                            
                            # Проверяем на коллизию со всеми врагами
                            for enemy_id, enemy in all_players.items():
                                ...
                                if enemy_id != p.id:
                                    ...
                                    if b_rect.colliderect(enemy_rect):
                                        ...
                                        # В текущей архитектуре мы просто удаляем пулю.
                                        # В реальной ММО здесь нужно отправить на сервер пакет HIT.
                                        
                                        break # Пуля попала в одного врага, можно не проверять других
                        
                        # 2. Удаляем пули, которые попали (они исчезают)
