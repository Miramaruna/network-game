...
def main_menu():
    user_ip = ""
    user_nick = "" # НОВОЕ: Переменная для ника
    clock = pygame.time.Clock()
    active_field = "IP" # НОВОЕ: Следим за активным полем ввода ("IP" или "NICK")
    active = True
    ...
    while active:
        ...
        # Определяем цвет рамки активного поля
        ...
        # Мигающий курсор для активного поля
        nick_text = user_nick
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_BACKSPACE:
                    ...
                    elif active_field == "NICK" and len(user_nick) < 12: # Ограничение ника до 12 символов
...
def game_loop(server_ip, nickname):
    ...
    while run:
        ...
        # Если игрок мертв, он не должен двигаться
        elif p.hp <= 0:
            # Сбросить движение, чтобы избежать отправки ненужных команд
            p.last_move = (0, 0)

        # --- СЕТЬ ---
        # Теперь отправляем весь объект p, включая его новый nickname
        ...
        for bullet in current_bullets:
            ...
            for other_id, other_p in all_players.items():
                # Проверяем попадание, если это не мы и цель жива (HP > 0)
                if other_id != p.id and other_p.hp > 0:
                    other_rect = pygame.Rect(other_p.x, other_p.y, other_p.width, other_p.height)
                    
                    if bullet_rect.colliderect(other_rect):
                        # 1. Удаляем пулю визуально у себя
                        p.deleteBullet(bullet)
                        
                        # 2. Готовим данные для сервера: "Я попал в ID такой-то"
                        hit_data.append({
                            "target_id": other_id,
                            "damage": 10 
                        })
        ...
        # dead = server_data.get("dead", bool)
        
        if p.id in all_players:
            server_p = all_players[p.id] # Получаем "себя" из данных сервер
            p.hp = server_p.hp     
        ...
        for p_id, player in all_players.items():
            ...
            # --- НОВОЕ: ОТОБРАЖАЕМ НИКНЕЙМ ---
            display_name = getattr(player, 'nickname', f"Игрок {p_id}") # Отображаем ник, если есть
...
def main_app():
    app_running = True
    while app_running:
        # --- ИЗМЕНЕНИЕ: main_menu теперь возвращает кортеж (IP, NICK) ---
        result = main_menu()
        
        if result == "QUIT": 
            app_running = False
        elif isinstance(result, tuple):
            server_ip, nickname = result # Распаковываем IP и Никнейм
            game_loop(server_ip, nickname) # Передаем оба значения
    pygame.quit()
