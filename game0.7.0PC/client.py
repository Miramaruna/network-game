...
pygame.display.set_caption("Neon Shooter: Ultimate UI")
...
def main_menu():
    ...
    # Данные
    user_ip = "127.0.0.1"
    user_nick = f"Neon_{random.randint(10,99)}"
    ...
    while True:
        ...
        # Заголовок
        title_surf, title_rect = FONT_BIG.render("NEON STRIKE", C_NEON_CYAN)
        # Эффект тени заголовка
        shadow_surf, _ = FONT_BIG.render("NEON STRIKE", (C_NEON_PINK))
...
def game_loop(server_ip, nickname, selected_skin, is_local_host):
    ...
    while run:
        ...
        # Сетка
        # start_x = -int(scroll[0]) % 100
        # start_y = -int(scroll[1]) % 100
        # for x in range(start_x, WIDTH, 100):
        #     color = (40, 40, 70)
        #     if abs(x + scroll[0]) < 5: color = C_NEON_PINK # Центр мира
        #     pygame.draw.line(win, color, (x, 0), (x, HEIGHT))
        # for y in range(start_y, HEIGHT, 100):
        #     color = (40, 40, 70)
        #     if abs(y + scroll[1]) < 5: color = C_NEON_PINK
