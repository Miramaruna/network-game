...
def main_menu():
    ...
    user_ip = "192.168.0.110" # По умолчанию localhost для удобства
    user_nick = ""
    active_field = "NICK"
    ...
    while True:
        ...
        inputs = [("NICKNAME", user_nick, 250, active_field == "NICK"), ("SERVER IP", user_ip, 350, active_field == "IP")]
...
def game_loop(server_ip, nickname, selected_skin):
    ...
    while run:
        ...
        # --- СБОР ДАННЫХ ДЛЯ ДЕБАГА (ЧТО ОТПРАВЛЯЕМ) ---
        ...
        # --- DRAW DEBUG OVERLAY (В самом конце перед курсором) ---
        debug_ui.draw(win, p, n, int(clock.get_fps()), scroll, last_packet_info)

        # draw_text_freetype(FONT_HUD, f"Pos: {p.x}, {p.y}", (255, 255, 0), WIDTH - 150, 10) # Убрали, так как теперь это в дебаге
        mx, my = pygame.mouse.get_pos()
