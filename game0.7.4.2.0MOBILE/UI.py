...
def get_font(size, bold=False):
    """
    Универсальная функция для Android и ПК. 
    Игнорируем 'name', так как на Android мы всегда берем наш файл.
    """
    # Путь к шрифту (на Android он в корне проекта в папке shrifts)
    font_path = os.path.join("shrifts", "Arls.otf")
    
    try:
        if os.path.exists(font_path):
            # Загружаем наш OTF файл
            return pygame.freetype.Font(font_path, size)
        else:
            # Если файла нет (например, при запуске на ПК без папки), 
            # берем любой системный
            return pygame.freetype.SysFont('sans-serif', size, bold=bold)
    except:
        # Полный бэкап, если всё сломалось
        return None
...
FONT_BIG    = get_font(35)
FONT_MED    = get_font(12, bold=True)
FONT_SMALL  = get_font(10)
FONT_UI     = get_font(10)
FONT_HUD    = get_font(15, bold=True)
FONT_TITLE  = get_font(35, bold=True)
FONT_DEBUG  = get_font(10)
FONT_TABLE  = get_font(13, bold=True)
...
class TouchControls:
    def __init__(self, screen_w, screen_h):
        self.update_size(screen_w, screen_h)
        self.joy_active = False
        self.joy_touch_id = None # Храним ID пальца

    def update_size(self, screen_w, screen_h):
        self.screen_w = screen_w
        self.screen_h = screen_h
        self.joy_center = pygame.Vector2(150, screen_h - 150)
        self.joy_pos = pygame.Vector2(self.joy_center)
        self.joy_radius = 75
        
        btn_s = 80
        self.btn_ability1 = pygame.Rect(screen_w - 220, screen_h - 120, btn_s, btn_s)
        self.btn_ability2 = pygame.Rect(screen_w - 110, screen_h - 220, btn_s, btn_s)
        self.btn_chat = pygame.Rect(screen_w - 100, 20, 80, 50)
        self.btn_exit = pygame.Rect(20, 20, 50, 50)

    def draw(self, surface):
        overlay = pygame.Surface((self.screen_w, self.screen_h), pygame.SRCALPHA)
        # Основание джойстика
        pygame.draw.circle(overlay, (255, 255, 255, 40), self.joy_center, self.joy_radius)
        # Ручка джойстика
        joy_col = C_NEON_CYAN if self.joy_active else (150, 150, 150)
        pygame.draw.circle(overlay, (*joy_col, 180), self.joy_pos, 35)
        
        # Кнопки
        for btn, label, col in [
            (self.btn_ability1, "SHIELD", C_NEON_CYAN),
            (self.btn_ability2, "WALL", C_NEON_PINK),
            (self.btn_chat, "CHAT", C_NEON_GREEN),
            (self.btn_exit, "ESC", C_DANGER)
        ]:
            pygame.draw.rect(overlay, (20, 20, 35, 200), btn, border_radius=12)
            pygame.draw.rect(overlay, col, btn, 2, border_radius=12)
            FONT_SMALL.render_to(overlay, (btn.centerx - 20, btn.centery - 5), label, col)
        
        surface.blit(overlay, (0, 0))

    def get_movement(self):
        if not self.joy_active: return 0, 0
        direction = self.joy_pos - self.joy_center
        if direction.length() > 5:
            return direction.normalize().x, direction.normalize().y
        return 0, 0
